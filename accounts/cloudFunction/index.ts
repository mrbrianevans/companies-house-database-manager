...
const uploadCsvToDb = async (event, context) => {
    ...
    console.log(companyNumber, ':', factsInserted, 'facts inserted;',
        longFactsInserted, 'long facts inserted; in', timeTakenToInsert, 'ms. Average', Math.round(timeTakenToInsert / (factsInserted + longFactsInserted) * 10) / 10, 'ms per insert' + (multiInsertError ? '' : ', Multi-insert used'))
