          ...
          .on('data', async (data) => {
              ...
              await client.query(companyQuery, Object.values(data).slice(0, 10))
                .then(async (res) => {
                    ...
                    if (sicOne)
                        await client.query(sicQuery, [data.number, sicOne[0]]).catch(err => {
                            if (err) sic_errors++
                        })
                    
                    const sicTwo = data.SicCode2.match(/^[0-9]{5}/)
                    if (sicTwo)
                        await client.query(sicQuery, [data.number, sicTwo[0]]).catch(err => {
                            if (err) sic_errors++
                        })
                    
                    const sicThree = data.SicCode3.match(/^[0-9]{5}/)
                    if (sicThree)
                        await client.query(sicQuery, [data.number, sicThree[0]]).catch(err => {
                            if (err) sic_errors++
                        })
