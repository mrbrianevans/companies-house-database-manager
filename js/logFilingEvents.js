              ...
              switch (r.statusCode) {
                  case 200:
                      ...
                      reportStatsInterval = setInterval(() => {
                          ...
                          let noColorMessage = `Alive: ${Math.round((Date.now() - startTime) / 1000)}s | Backlog: ${Math.round(averageBacklog)}s | proc/note ${Math.round(recentProcessingTimePerNotification / averageTimePerNewNotification * 100)}% | Processing time: ${Math.round(recentProcessingTimePerNotification)}ms | New notification every: ${Math.round(averageTimePerNewNotification)}ms | Events Saved: ${eventsSaved} | Database errors: ${databaseErrors}`
